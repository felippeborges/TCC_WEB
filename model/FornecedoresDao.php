...
class FornecedoresDao {
    ...
    public function m_BuscarFornecedorPorCodigo(FornecedoresBean $fornecedor) {
        try {
            ...
            ."f.for_contato1,"
            ."f.for_email,"
            ."f.for_cnpjcpf FROM fornecedores f left outer join cidades c on c.cid_codigo = f.cid_codigo where for_codigo = :for_codigo ";
            $statement_sql = ConexaoPDO::getInstance()->prepare($sql);
            $statement_sql->bindValue(":for_codigo", $fornecedor->getFor_codigo());
    ...
    public function m_buscaTodosFornecedores() {
        $sql = "SELECT f.for_codigo,"
                ...
                . "f.for_contato1,"
                . "f.for_email,"
                . "f.for_cnpjcpf FROM fornecedores f left outer join cidades c on c.cid_codigo = f.cid_codigo";
        $statement_sql = ConexaoPDO::getInstance()->prepare($sql);
        $statement_sql->execute();
